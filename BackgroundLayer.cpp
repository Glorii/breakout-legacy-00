...
void BackgroundLayer::initPhysics(){
...
void BackgroundLayer::initDebugDraw(){
...
void BackgroundLayer::calInterPoint(){
...
void BackgroundLayer::calReflectedRay(){
    ...
    //b2Vec2 lowerVec = paddleBody->GetFixtureList()->GetAABB(0).lowerBound;
    //b2Vec2 upperVec = paddleBody->GetFixtureList()->GetAABB(0).upperBound;
    //b2Vec2 extVec = paddleBody->GetFixtureList()->GetAABB(0).GetExtents();
    //b2Vec2 extSkewVec = extVec.Skew();
    
    
    //b2Vec2 verticalVec = paddleBody->GetWorldVector(b2Vec2(1.0f, 0.0f));
    
    
    
    //Vec2 normalVec;
    //normalVec.x = verticalVec.x;
    //normalVec.y = verticalVec.y;
    vec1.subtract(vec2);
    Vec2 normalVec = vec1;
    normalVec.normalize();
...
void BackgroundLayer::update(float dur){
    int velIteration = 8;
    int postIteration = 3;
...
void BackgroundLayer::addPaddle(){
    ...
    paddleShape.SetAsBox( PADDLE_THICKNESS/2 / WORLDSCALE,440/2 / WORLDSCALE);
...
void BackgroundLayer::addBrick(int i, int j){
...
void BackgroundLayer::addBrick(float posx,float posy,std::string filename,std::string name )
{
...
void BackgroundLayer::addMonsterBody(){
...
void BackgroundLayer::initMonsterBody(){
	if (monster)
	{
		removeChild(monster);
	}
...
void BackgroundLayer::dealDeletion(float dt){
...
void BackgroundLayer::dealCollision(float dur){
    for (b2ContactEdge *c = monsterBody->GetContactList();c;c = c->next){
        ...
        if (sp && (/*sp->getName() == "brick" ||*/ sp->getName() == "wall" || sp->getName() == "baffle") && contact->IsTouching() == true) {
			...
			/*if (sp->getName() == "brick")
			{
				...
				this->scheduleOnce(schedule_selector(BackgroundLayer::dealDeletion),0.05f);
			}*/
...
void BackgroundLayer::BeginContact(b2Contact* contact){

}

void BackgroundLayer::EndContact(b2Contact* contact){
...
void BackgroundLayer::onTouchMoved( Touch* touch, Event *event)
{
    auto location = touch->getLocation();
...
void BackgroundLayer::onTouchEnded( Touch* touch, Event *event )
{
    if(m_bMouseDown){
        m_bMouseDown=false;
        m_vec2_1->updatefunc();
...
void BackgroundLayer::initBrickGroup()
{
	for (int i = 0; i < BRICK_COLUMN; i++) {
		for (int j = 0; j < BRICK_COLUMN; j++) {
			//addBrick(i, j);
            
            if (bricks[i + j * BRICK_COLUMN] == NULL) {
                bricks[i + j * BRICK_COLUMN] = new BasicBrick(world,visibleSize,this);
                bricks[i + j * BRICK_COLUMN]->init(i, j);
            }
