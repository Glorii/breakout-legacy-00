...
ParentOfPieces::ParentOfPieces(){
}

ParentOfPieces::~ParentOfPieces(){
}

bool ParentOfPieces::init(){
    if(!Layer::init())
    {
        return false;
    }
...
void ParentOfPieces::initGame(){
    ...
    listener->onTouchBegan = CC_CALLBACK_2(ParentOfPieces::onTouchBegan, this);
    listener->onTouchMoved = CC_CALLBACK_2(ParentOfPieces::onTouchMoved, this);
    listener->onTouchEnded = CC_CALLBACK_2(ParentOfPieces::onTouchEnded, this);
...
void ParentOfPieces::initPhysics(){
...
void ParentOfPieces::initDebugDraw(){
...
void ParentOfPieces::draw(Renderer *renderer, const Mat4 &transform, uint32_t flags){
...
void ParentOfPieces::calInterPoint(){
...
void ParentOfPieces::calReflectedRay(){
...
void ParentOfPieces::update(float dur){
    int velIteration = 8;
    int postIteration = 3;
...
void ParentOfPieces::addBrick(int i, int j){
...
void ParentOfPieces::addBrick(float posx,float posy,std::string filename,std::string name )
{
...
void ParentOfPieces::addMonsterBody(){
...
void ParentOfPieces::initMonsterBody(){
    if (monster)
    {
        removeChild(monster);
    }
...
void ParentOfPieces::dealDeletion(float dt){
...
void ParentOfPieces::dealCollision(float dur){
    ...
    for (b2ContactEdge *c = monsterBody->GetContactList();c;c = c->next){
        ...
        if (sp && (sp->getName() == "brick" || sp->getName() == "wall") && contact->IsTouching() == true) {
            ...
            /*sp->setTag(sp->getTag() - 1);
             ...
             this->scheduleOnce(schedule_selector(ParentOfPieces::dealDeletion),0.05f);
             }*/
...
void ParentOfPieces::BeginContact(b2Contact* contact){
    
}

void ParentOfPieces::EndContact(b2Contact* contact){
    
}

bool ParentOfPieces::onTouchBegan( Touch* touch, Event *event )
{
...
void ParentOfPieces::onTouchMoved( Touch* touch, Event *event)
{
    auto location = touch->getLocation();
...
void ParentOfPieces::onTouchEnded( Touch* touch, Event *event )
{
    if(m_bMouseDown){
        m_bMouseDown=false;
        m_vec2_1->updatefunc();
...
void ParentOfPieces::initBrickGroup()
{
...
void ParentOfPieces::onUpdate()
{
    
}

void ParentOfPieces::onDealCollision(b2Body* collidedBody)
{
    
}

void ParentOfPieces::removeJudgment( b2Body* collidedBody)
{
    Sprite *sp = (Sprite *)collidedBody->GetUserData();
